<template>
  <div id="home">
   ...
   <Cards style="float:left;padding-bottom:20px" v-for="list in categoriesListfilter" :key="list.handle" :list="list"></Cards>
    <LazyHydrate when-visible>
      <TrendingProducts :products="categories.products" :title="categories.title"/>
    ...
    <!-- <LazyHydrate when-visible>
      <BestSeller :products="categories.products" />
    </LazyHydrate> -->
...
export default {
  ...
  setup() {
    const { products: bestSeller, search: bestSellerSearch } = useProduct('all-products');
    const { categories, search } = useCategory("categories");
    const {categories:categoriesList,search: searchList} = useCategory('all-categories');
    const {
    ...
    onSSR(async () => {
      await bestSellerSearch({slug:'my-product'});
      await searchList({slug:''})
      await search({ slug: "trending-collections" });
      await productsSearch({ limit: 16 });
      await loadCart();
    });
    return {
      products: computed(() => 
  ...
  computed:{
    categoriesListfilter(){
      return this.categoriesList.filter((item)=>item.handle === 'featured-collection' || item.handle ==='shoes')
    }
  },
  methods: {
    // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
  ...
  mounted() {
    // console.log(productGetters.getFiltered(this.categories[0]))
    // this.categoriesList.filter((item)=>item.handle === 'featured-collection' || item.handle ==='shoes');
